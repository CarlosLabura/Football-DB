...
class Date:
    def compare(date1:Date, date2:Date) -> bool:
        """ Compares if date1 is higher than date2 | Returns: Boolean or None (if they are equals) """
    ...
    def age(date:Date, current:Date) -> int:
        """ Returns the difference between the first date and the second date (which is usally the current one) | Returns: The diference (int) """
    ...
    def birthday(date:Date, current:Date) -> bool:
        """ Checks if the first date is the same as the second date (which is usally the current one) | Returns: Boolean with the comparison """
        if type(date) != Date or type(current) != Date:
    ...
    def __init__(self, day:int=1, month:int=1, year:int=2000) -> Date:
        """ Starts a Date object | Returns: self """
        self.setYear(year)
        self.setMonth(month)
        self.setDay(day)
    def __str__(self):
    ...
    def setYear(self, year:int=2000) -> int:
        """ Sets the date year, also checking if its a leap year | Returns: current year after the method """
        year = int(year)
    ...
    def setMonth(self, month:int=1) -> int:
        """ Sets the date month, also checking the days each month has | Returns: current month after the method """
        month = int(month)
    ...
    def setDay(self, day:int=1) -> int:
        """ Sets the date day | Returns: current day after the method """
        day = int(day)
        
        self.day = max(1, min(self.monthLimit, day))
        return self.day
    def passMonths(self, times:int=1) -> int:
        """ Passes months by the times asked, also passings years if needed | Returns: current month after the method """
    ...
    def passDays(self, times:int=1) -> int:
        """ Passes days by the times asked, also passing months if needed | Returns: current day after the method """
        self.day += int(times)
